...
include '../../config.php';
$userId = $_SESSION['aId'];
$sl_id = mysqli_query($conn, "select * from admin where id = " . $userId);
					...
					<div class="row ">
						<div class="col-5 border m-2 p-4 rounded-5">
							...
							<div class="row p-3  rounded-4 ">
								<div class="col-2 p-2">
									<img
										src="<?php echo htmlspecialchars($_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['HTTP_HOST'] . '/trust/api/' . $photo); ?>"
										alt="avatar"
										class="avatar"
										style="height: 70px; width: 70px; border-radius:50%"
									/>
								</div>
								<div class="col-8 m-3 p-2">
									<center>
										<button type="button" class="btn btn-primary" onclick="openModal1()">Upload
											Profile</button>
									</center>
								</div>
							</div>
							<div class="p-2 row border rounded-4 mt-2 text-start" style="font-size:14px;">
								...
								<center>
									<button type="button" class="btn btn-warning" style="width:150px; margin:10px;"
										onclick="openModal(1)"><i class='bx bx-edit '></i> Edit</button>
									...
									<div class="modal-content">
										...
										<script>
											document.getElementById("editPhotoForm").addEventListener("submit", function (event) {
												...
												fetch(commonApiPath + 'adminPhoto.php', {
													method: 'POST',
													body: formData
												})
													.then(response => response.json())
